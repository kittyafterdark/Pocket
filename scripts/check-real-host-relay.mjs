...
assert.ok(relay.injectedAt, 'Pocket did not record injecting this relay into the generation prompt.')
assert.equal(relay.injectedGenerationId, continuation.generationId, 'The relay injection was associated with a different generation.')
assert.match(relay.serializedRelay || '', /POCKET CONTINUITY RELAY — NEWER STATE/, 'The serialized urgent relay block was not retained for inspection.')
assert.ok(relay.serializedRelayChars > 0 && relay.serializedRelayChars <= 3_600, 'The relay context was missing or exceeded its total budget.')
assert.ok(relay.relayExchangeMessageCount > 0, 'The relay did not include any recent conversation messages.')
