...
async function applyAction(input, userId, source = "model") {
  ...
  const candidateMessageProvisional = source === "model" && action === "message" && Boolean(candidateOrigin(input.__candidateOrigin, context.chatId)) && !bool2(input.__candidateCommitted);
  if (action === "camera") {
...
function pocketArtifactTag(activityId, action) {
  ...
  return value && text2(action, 40) === "message" ? pocketInlineAnchor(value) : undefined;
}
