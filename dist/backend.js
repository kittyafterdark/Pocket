...
function stripMachineWrappers(value) {
  ...
  for (let pass = 0;pass < 3; pass += 1) {
    const next = text2.replace(FENCED_BLOCK, "").replace(WRAPPED_BLOCK, "").replace(POCKET_ACTION_BLOCK, "").replace(POCKET_ACTION_SINGLE, "").replace(POCKET_ARTIFACT_BLOCK, "").replace(POCKET_ARTIFACT_SINGLE, "");
    if (next === text2)
      break;
    text2 = next;
  }
...
async function reconcilePostTurnCandidate(chatId, characterId, origin, generationType, userId, options = { auditClock: false }) {
  ...
  const currentNarrative = sanitizeNarrativeContent(current?.content, 6000);
  if (!currentNarrative)
    return;
  const state = await loadState(chatId, characterId, userId);
  ...
  const recentNarrative = contextMessages.map((message, index) => {
    ...
    return `${role} [${index + 1}]: ${sanitizeNarrativeContent(message?.content, 1500)}`;
  }).join(`
...
function actionSemanticKey(action, input, payload) {
  ...
  for (const key of ["type", "requestId", "request_id", "commandId", "command_id", "idempotencyKey", "chatId", "chat_id", "characterId", "character_id", "messageId", "message_id", "payload", "__candidateOrigin", "__recoveredFromNarrative", "recoveryIndex"])
    delete merged[key];
...
function pocketArtifactTag(activityId) {
...
spindle.on("TOOL_INVOCATION", async (payload, eventUserId) => {
  ...
  try {
    ...
    return JSON.stringify({
      ...result,
      artifactTag: pocketArtifactTag(result.activityId),
