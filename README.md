...
When a Pocket Action message call succeeds, its JSON result includes an `artifactTag` such as `<pocket-artifact ref="activity_123"></pocket-artifact>`. For a message that is visible/readable in the scene, the RP model places that exact tag on its own line instead of quoting or styling the message body in prose. After the candidate commits, Pocket rewrites the tag in that exact swipe slot to a durable empty `data-pocket-inline-anchor`; the frontend mounts the canonical Pocket artifact into that anchor. This gives the renderer the model-chosen narrative position while keeping the phone payload authoritative in Pocket. Missing tags degrade to a compact end-of-message provenance receipt rather than a second fake phone card. Swipe projection clears injected receipts and remounts only activities belonging to the selected candidate.
