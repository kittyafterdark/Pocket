...
A local handoff writes an already-occurred Timeline event and a bounded pending continuity relay, then uses Lumiverse's native `spindle.chat.appendMessage(..., { triggerGeneration: true })` path to continue the main RP. Every relay retains its creating burst ID. The interceptor injects only the relay matched to that generation by Pocket message metadata or generation ID; ordinary generations do not receive the entire pending queue. A relay is consumed only by its matching successful generation and remains retryable after blocked permissions, host failure, or stop. Its diagnostics distinguish invocation, permission checks, host acceptance, and the observed `GENERATION_STARTED` event. A local conversation hides autonomous reply affordances and offers **Return to roleplay**, the Timeline handoff, and an explicit **Message anyway** override. Leaving the scene restores remote eligibility without erasing an unrelated pause.
