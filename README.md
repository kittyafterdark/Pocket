...
When a Pocket Action message call succeeds inside a host generation, its side effect is **provisional** until the final assistant candidate commits it. The tool result provides both an `artifactTag` such as `<pocket-artifact ref="activity_123"></pocket-artifact>` and a hidden `commitTag`. Visible/readable/observed phone actions use the `artifactTag` exactly where the artifact belongs; genuinely off-screen actions use the `commitTag`; reasoning-only tool calls use neither and are deleted at candidate commit. Provisional messages are excluded from Pocket projection and model-facing phone context, so refreshing Pocket mid-generation cannot make exploratory tool calls canon.

After commit, Pocket rewrites a valid `artifactTag` in that exact swipe slot to a durable empty `data-pocket-inline-anchor`; the frontend mounts the canonical Pocket artifact into that anchor. If the model ignores the tag but prints the exact canonical Pocket message as a standalone quoted/styled line, Pocket can rescue that line by replacing it with the same inline anchor instead of leaving fake plaintext plus a footer card. Missing placement evidence degrades to a compact provenance receipt rather than guessing a paragraph location. Swipe projection republishes on every host navigation and clears stale receipts/inline surfaces; deleted swipes use Lumiverse's `deleted` action semantics, and deleting a host assistant message removes every Pocket message/activity/notification and candidate clock owned by that turn.
