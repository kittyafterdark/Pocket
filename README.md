...
Group conversations use a single structured generation to produce an ordered 0–3-message burst (hard maximum 4). Auto mode supplies only eligible off-scene participants and their independent talkativeness/fragmentation styles; later array entries can react to earlier entries. Pocket owns reveal timing, and queued entries are not canonical messages until their reveal. A new user message preserves anything already delivered while cancelling the stale remainder. Per-conversation flight guards prevent overlapping batches. Choosing a specific participant from the compact **Auto speaker** sheet instead requests exactly one reply and clears that one-shot selection afterward. Group bubbles resolve each sender's current Contact accent and avatar at render time, so editing a Contact immediately restyles existing history without rewriting messages.
